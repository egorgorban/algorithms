...
for v in range(_from, _to):
    ...
    # начинаем замерять время работы функции, определяющей класс временной сложности
    t1 = time()
    for _ in range(stop):
        ...
        for i in range(3):
            s = i*(i)+1
            s = log2(s)
    # заканчиваем замерять время работы функции, определяющей класс временной сложности
    t2 = time() - t1
